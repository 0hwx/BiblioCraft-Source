...
public class ItemAtlas extends ItemMap {
    ...
    private int createUniqueID() {
        ...
        return Math.abs((int) (rando.nextLong() * MinecraftServer.getSystemTimeMillis()));
    }
    ...
    public boolean onItemUse(ItemStack itemStack, EntityPlayer player, World world, int x, int y, int z, int side, float hitX, float hitY, float hitZ) {
        if (player.isSneaking())//player.getHeldItem(EnumHand.OFF_HAND) == null)
        {
                    ...
                    if (mapFrame.getHasMap()) {
                        ...
                        if (matchingMapSlot == -1) {
                            ...
                            if (!alreadySetToMap) {
                                ...
                                BiblioNetworking.INSTANCE.sendTo(new BiblioAtlasTGUI(stack, x, y, z), (EntityPlayerMP) player);
                        ...
                        if (checkForAndRemoveStackedEmptyMap(stack, player)) {
                            ...
                            if (mapData != null && atlasMapStack != null) {
    ...
    private boolean addnewMapToAtlasFromFrame(ItemStack stack, TileEntityMapFrame tile, EntityPlayer player) {
        ...
        NBTTagCompound tags = stack.getTagCompound();
        if (inv != null && newMapSlot != -1 && newMap != null && tags != null) {
            String newMapName = "Map_" + newMap.getItemDamage();
            if (inv.getStackInSlot(newMapSlot) == null) {
                int emptyMapsSlot = getEmptyStackedMapSlot(stack);
                if (emptyMapsSlot != -1) {
                    ItemStack currentSlot = inv.getStackInSlot(emptyMapsSlot);
                    boolean vanillaTest = currentSlot != null && currentSlot.getItem() == Items.map;
                    boolean tfcTest = currentSlot != null && currentSlot.getItem() == Items.paper && Loader.isModLoaded("terrafirmacraft") && Loader.isModLoaded("BiblioWoodsTFC");
                    if (vanillaTest || tfcTest) {
                        currentSlot.stackSize = (currentSlot.stackSize - 1);
...
//	    	}
    ...
    private NBTTagCompound getNewMapDataCompound(TileEntityMapFrame tile, String newMapName) {
        ...
        for (int i = 0; i < tile.xPin.size(); i++) {
            ...
            if (tile.getVertPosition() == EnumVertPosition.WALL && (tile.getAngle() == ForgeDirection.WEST || tile.getAngle() == ForgeDirection.NORTH)) {
                xpin = 1.0f - xpin;
            }
    ...
    private int findMapSlotForNewMap(ItemStack stack) {
        ...
        if (inv != null) {
            for (int i = 6; i < inv.getSizeInventory(); i++) {
                ...
                boolean vanillaTest = mapStack != null && mapStack.getItem() == Items.map;
                boolean tfcTest = mapStack != null && mapStack.getItem() == Items.paper && Loader.isModLoaded("terrafirmacraft") && Loader.isModLoaded("BiblioWoodsTFC");
                if (vanillaTest || tfcTest) {
                    return i;
                }
            }
    ...
    private int findMatchingMapSlot(ItemStack stack, TileEntityMapFrame tile) {
        ...
        if (frameMap != null && frameMap.getItem() == Items.filled_map && inv != null && atlasTags != null) {
            ...
            for (int n = 6; n < inv.getSizeInventory(); n++) {
                ...
                if (atlasStack != null && atlasStack.getItem() == Items.filled_map) {
    ...
    public ItemStack onItemRightClick(ItemStack itemStackIn, World world, EntityPlayer player) {
        ...
        if (player.getHeldItem() == null || player.getHeldItem().getItem() == null) {
            ...
            if (gui == 0) {
                ...
                if (world.isRemote) {
                    openMapGUI(world, player, player.getHeldItem());
                }
            }
        }
...
//			    		}
    ...
    private boolean checkIfPlayerIsHoldingAtlas(EntityPlayer player) {
        ...
        if (hand.getItem() instanceof ItemAtlas)// ||   the problem is I put the atlas back in the main hand.
        {
            return true;
        }
        return false;
...
//	    		}
    ...
    public static int setAtlasDamage(ItemStack atlas, int newslot) {
        ...
        if (newmap != null && newmap.getItem() instanceof ItemMap) {
...
//	                mapdata.trackingPosition = true;
    ...
    public void addInformation(ItemStack stack, EntityPlayer player, List<String> tooltip, boolean flagIn) {
        ...
        NBTTagList nbttaglist = Items.enchanted_book.func_92110_g(stack);
        ...
        for (int i = 0; i < nbttaglist.tagCount(); ++i) {
            ...
            if (Enchantment.enchantmentsList[short1] != null) {
                tooltip.add(Enchantment.enchantmentsList[short1].getTranslatedName(short2));
            }
        }
    }
    @Override
    public void registerIcons(IIconRegister register) {
        this.itemIcon = register.registerIcon("bibliocraft:atlas");
    }
}
