...
public class BiblioSortingHelper
{
			...
			if (thing != null)
			{
				...
				for (int n = 0; n < chestOfStuff.size(); n++)
				{
					...
					if (oldThing != null)
					{
						...
						{
							oldThing.stackSize = (oldThing.stackSize + thing.stackSize);
							int extras = getExtraStackQuanityFromSpecialInventories(tile, oldThing.stackSize, i);
							if (extras != -1)
							{
								oldThing.stackSize = (extras);
				...
				if (!alreadyInList)
				{
					...
					int extras = getExtraStackQuanityFromSpecialInventories(tile, newThing.stackSize, i);
					if (extras != -1)
					{
						newThing.stackSize = (extras);
	...
	public static ArrayList<ItemStack> getSpecialInventoryQuanities(TileEntity tile)
	{
		...
		{
			// Storage Drawers
			if (tags.hasKey("id") && tags.getString("id").contentEquals("storagedrawers:basicdrawers") && tile instanceof IInventory)
			{
				IInventory handler = (IInventory) tile;
				...
				if (tagList != null)
				{
					...
					for (int i = 0; i < tagList.tagCount(); i++)
					{
						...
						ItemStack thing = handler.getStackInSlot(i);
						thing.stackSize = (slotInfo.getInteger("Count"));
	...
	public static ItemStack getLargestStackInList(ArrayList list)
	{
		...
		for (int i = 0; i < list.size(); i++)
		{
			...
			if (stack != null)
			{
				if (bestStack != null)
				{
					if (stack.stackSize > bestStack.stackSize)
					{
						bestStack = stack;
					}
				}
	...
	public static int getLargestStackSlotInList(ArrayList list)
	{
		...
		for (int i = 0; i < list.size(); i++)
		{
			...
			if (stack != null)
			{
					if (stack.stackSize > testStackSize)
					{
						testStackSize = stack.stackSize;
						output = i;
					}
			}
		}
		...
		for (int i = 0; i < inventoryset.size(); i++)
		{
			...
			if (invSet != null)
			{
				...
				for (int j = 0; j < inv.size(); j++)
				{
					...
					if (stack != null)
					{
						InventoryListItem newInvListItem = new InventoryListItem(stack.getUnlocalizedName(), stack.stackSize, invSet.inventoryName, invSet.tileX, invSet.tileY, invSet.tileZ);
						...
						if (!listHasItem)
						{
							...
							listOfInventories.add(newInvListItem);
							itemList.add(new SortedListItem(stack.getUnlocalizedName(), stack.stackSize, listOfInventories));
						}
					}
				}
			}
	...
	public static ArrayList<SortedListItem> getSortedListByAlphabet(ArrayList<SortedListItem> list)
	{
		...
		for (int i = 0; i < list.size(); i++)
		{
			...
			if (item != null)
			{
				localizedNameList.add(I18n.format(item.itemName+".name"));
			}
		}
		...
		for (int i = 0; i < localizedNameList.size(); i++)
		{
			...
			for (int j = 0; j < list.size(); j++)
			{
				...
				if (localName.contentEquals(I18n.format(item.itemName+".name")))
				{
					sortedList.add(item);
					break;
				}
	...
	public static void listTester(ArrayList<SortedListItem> list)
	{
		for (int i = 0; i < list.size(); i++)
		{
			...
			if (item != null)
			{
				System.out.println("item name: "+I18n.format(item.itemName+".name")+"      Quantity: "+item.itemQuantity+"      Inside x inventories: "+item.inventoryList.size());
			}
		}
	}
}
