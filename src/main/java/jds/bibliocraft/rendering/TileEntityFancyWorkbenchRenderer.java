...
public class TileEntityFancyWorkbenchRenderer extends TileEntityBiblioRenderer implements IItemRenderer  {
    ...
    public void renderTileEntityAt(BiblioTileEntity tileEntity, double x, double y, double z, float tick) {
        ...
        if (tile != null) {
            ...
            renderFancyWorkbench(x + 0.5D, y, z + 0.5D, rotation, this.meta,false);

            GL11.glPopMatrix();
        }
            renderSlotItem(tile.getStackInSlot(0), 0.2, 1.01, 0.4, 0.8f,90.0F);
        }

    @Override
    public void additionalGLStuffForItemStack () {}

    public void renderFancyWorkbench(double x, double y, double z, double rotate, int meta, boolean renderAllBooks) {
    ...
    public void renderItem(ItemRenderType type, ItemStack item, Object... data) {
        switch (type) {
            case INVENTORY:
                renderFancyWorkbench(0, -0.5D, 0,180.0D, item.getItemDamage(),true);
                return;
            case EQUIPPED_FIRST_PERSON:
                renderFancyWorkbench(0.5D, 0, 0.5D, 90.0D, item.getItemDamage(),true);
                return;
            case EQUIPPED:
                renderFancyWorkbench(0.5D, 0, 0.5D, 0, item.getItemDamage(),true);
                return;
            default:
                renderFancyWorkbench(0, 0, 0, 0, item.getItemDamage(),true);
        }
    }
}
