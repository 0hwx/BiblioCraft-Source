...
import jds.bibliocraft.utils.math.Matrix4d;
import jds.bibliocraft.utils.math.VecmathUtil;
import jds.bibliocraft.utils.math.Vector.Vector2f;
import jds.bibliocraft.utils.math.Vector.Vector3d;
import jds.bibliocraft.utils.math.Vector.Vector3f;
import jds.bibliocraft.utils.math.Vector.Vector4d;
import jds.bibliocraft.utils.math.Vector.Vector4f;
import jds.bibliocraft.rendering.isbrh.Vertex.Vertex;
...
//            Log.error("Failed to initialize timer reflection for IO config.");
    ...
    /**
     * Returns a brightness multiplier for a rotated normal.
     * Works with rotated or partial-block models.
     *
     * @param normal The face normal as a normalized Vector3f
     * @return brightness multiplier in [0.2, 1.0]
     */
    public static float getColorMultiplierForFace(Vector3f normal) {
        // Treat y as vertical, x/z as horizontal
        float yFactor = 0.5f + 0.5f * normal.y; // up faces brightest, down darkest
        float xzFactor = 0.6f + 0.2f * (Math.abs(normal.x) + Math.abs(normal.z)); // sides slightly brighter if tilted
        return clamp(yFactor * xzFactor, 0.2f, 1.0f);
    }
