	...
	public boolean onBlockActivatedCustomCommands(World world, int x, int y, int z, EntityPlayer player, int side, float hitX, float hitY, float hitZ)
	{
		if (world.isRemote && player.isSneaking())
		{
			...
			if (tile != null && tile instanceof TileEntityClock)
			{
				...
				{
					TileEntity upperTile = world.getTileEntity(x, y + 1, z);
...
//	}
	...
	public AxisAlignedBB getCollisionBoundingBoxFromPool(World world, int x, int y, int z)
	{
    	...
    	if (te != null && te instanceof TileEntityClock)
    	{
    		...
    		ForgeDirection angleGet = tile.getAngle();
	...
	public void breakBlock(World world, int x, int y, int z, Block blockBroken, int meta)
	{
		...
		if (tile != null && tile instanceof TileEntityClock)
		{
			...
			{
				TileEntityClock topClock = (TileEntityClock)world.getTileEntity(x, y + 1, z);
				topClock.setVertPosition(EnumVertPosition.WALL);
			}
			else if (clock.getVertPosition() == EnumVertPosition.CEILING)
			{
				TileEntityClock bottomClock = (TileEntityClock)world.getTileEntity(x, y - 1, z);
