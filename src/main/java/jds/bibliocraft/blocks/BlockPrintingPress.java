...
//	public static final PropertyEnum COLOR = PropertyEnum.create("color", EnumColor.class);
	...
	public boolean onBlockActivatedCustomCommands(World world, int x, int y, int z, EntityPlayer player, int side, float hitX, float hitY, float hitZ)
	{
		...
		if(!world.isRemote && t != null && t instanceof TileEntityPrintPress)
		{
			...
			int slot = getSlot(tile, ForgeDirection.getOrientation(side), hitX, hitZ);
			...
			switch (slot)
			{
				...
				{
					//plate
					if (playerhand != null && (playerhand.getItem() instanceof ItemPlate || playerhand.getItem() instanceof ItemEnchantedPlate || playerhand.getItem() instanceof ItemAtlasPlate))
					{
						tile.addStackToInventoryFromWorld(playerhand, slot, player);
					}
					else
				...
				{
					//input
					if (playerhand != null && (playerhand.getItem() instanceof ItemBook || playerhand.getItem() instanceof ItemAtlas))
					{
						tile.addStackToInventoryFromWorld(playerhand, slot, player);
					}
					else
	...
	public static int getSlot(TileEntityPrintPress tile, ForgeDirection face, float hitX, float hitZ)
	{
		int output = -1;
